...
class Channels extends React.PureComponent<Props, State> {
  state: State = {
    activeChannel: "",
  ...
  addListeners = () => {
    let loadedChannels: CurrentChannel[] = [];
    this.state.channelsRef.on("child_added", (snap) => {
      loadedChannels.push(snap.val());
      this.setState({ channels: loadedChannels }, () => this.setFirstChannel());
      this.addNotificationListener(snap.key as string);
    });
  };

  addNotificationListener = (channelId: string) => {
  ...
  handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    const name = event.target.name as "channelName" | "channelDetails";
  ...
  getNotificationCount = (channel: CurrentChannel) => {
    let count = 0;
