...
type Props = {
  ...
  setUserPosts: typeof setUserPosts;
};
...
type typingUsers = { id: string | null; name: string | null };
...
class Messages extends React.Component<Props, State> {
  ...
  removeListeners = (listeners: listeners[]) => {
  ...
  countUserPosts = (messages: MessageType[]) => {
    let userPosts = messages.reduce((acc: UserPosts, message) => {
  ...
  displayMessages = (messages: MessageType[]) =>
    messages.length > 0 &&
  ...
  displayTypingUsers = (users: typingUsers[]) =>
    users.length > 0 &&
    users.map((user) => (
      <div
  ...
  render() {
    // prettier-ignore
    const { messages, channel, user, numUniqueUsers, searchTerm, searchResults, searchLoading, privateChannel, isChannelStarred, typingUsers, messagesLoading } = this.state;
